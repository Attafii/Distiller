  ...
  const handleSocialLogin = async (provider: "google" | "github") => {
    try {
      const res = await fetch("/api/auth/sign-in/social", {
        ...
        body: JSON.stringify({ provider, callbackURL: callbackUrl })
      });

      if (!res.ok) {
        throw new Error(await readErrorMessage(res, `${provider} sign-in failed`));
      }
      ...
      <Card className="border-border bg-card shadow-soft">
        <CardHeader className="text-center pb-2">
          ...
          <CardDescription className="text-sm text-muted-foreground">
            Sign in to your Distiller account
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4 pt-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Button variant="outline" type="button" className="w-full gap-2" onClick={() => handleSocialLogin("google")}>
            ...
            <Button variant="outline" type="button" className="w-full gap-2" onClick={() => handleSocialLogin("github")}>
